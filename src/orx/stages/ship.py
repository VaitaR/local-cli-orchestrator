...
class ShipStage(BaseStage):
    ...
    def execute(self, ctx: StageContext) -> StageResult:
        ...
        if create_pr and auto_push:
            log.info("Creating PR", base_branch=base_branch)
            try:
                ...
                if pr_url:
                    ...
                    log.info("PR created successfully", url=pr_url)
            except Exception as e:
    ...
    def _create_pr(
        ...
        # Read PR body and task
        pr_body = ctx.pack.read_pr_body() or "Implementation changes"
        task = ctx.pack.read_task() or "Implementation"

        # Add footer with agent metadata
        agent_footer = f"""

---
*This PR was automatically generated by ORX agent.*
- **Run ID:** {ctx.paths.run_id}
- **Engine:** {ctx.config.get("engine", {}).get("type", "unknown")}
- **Timestamp:** {ctx.pack.read_task() and "See run logs for details" or ""}
"""
        pr_body_with_footer = pr_body + agent_footer
